...
class DialogueCharacterEditorState extends MusicBeatState
{
	...
	private static var mewheniseemagnus:String = 'FUCK YOU FUCKIN DICK! ALWAYS NAYSAYING! EVERYTHING I CREATE! YOU PEICE OF SHIT! YOU CREATE SOMETHING!';
	function reloadText() {
		if(daText != null) {
			daText.killTheTimer();
			daText.kill();
		...
		daText = new Alphabet(0, 0, mewheniseemagnus, false, true, 0.05, 0.7);
		daText.x = DialogueBoxPsych.mewheniseemagnus_X;
		daText.y = DialogueBoxPsych.mewheniseemagnus_Y;
		hudGroup.add(daText);
	}
	...
	override function update(elapsed:Float) {
		...
		if(!blockInput && !animationDropDown.dropPanel.visible) {
			...
			if(FlxG.keys.justPressed.ESCAPE) {
				...
				FlxG.sound.playMusic(Paths.music('OUAWAAWWAAWAWAW'), 1);
				transitioning = true;
			}
