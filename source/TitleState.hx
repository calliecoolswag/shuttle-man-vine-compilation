...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		if(!closedState) {
			...
			var http = new haxe.Http("https://raw.githubusercontent.com/eggvipers/shuttle-man-vine-compilation/main/Version.txt");
	...
	function startIntro()
	{
		if (!initialized)
		{
			...
			// music.loadStream(Paths.music('OUAWAAWWAAWAWAW'));
			// FlxG.sound.list.add(music);
			// music.play();

			if(FlxG.sound.music == null) {
				FlxG.sound.playMusic(Paths.music('OUAWAAWWAAWAWAW'), 0);

				FlxG.sound.music.fadeIn(1.3, 0, 0.7);
			}
		}
		...
		bob = new FlxSprite(0, FlxG.height * 0.52).loadGraphic(Paths.image('bob_mod'));
		add(bob);
		bob.visible = false;
		...
		kys = new FlxSprite(0, FlxG.height * 0.52).loadGraphic(Paths.image('kys'));
		add(kys);
		kys.visible = false;
		kys.setGraphicSize(Std.int(kys.width * 0.8));
		kys.updateHitbox();
		kys.screenCenter(X);
		kys.antialiasing = true;
